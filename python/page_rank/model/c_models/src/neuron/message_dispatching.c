...
bool message_dispatching_initialise(address_t synaptic_matrix_address,
    ...
    // Work out the positions of the direct and indirect synaptic matrices and
    // copy the direct matrix to DTCM
...
bool message_dispatching_process_synaptic_row_page_rank(synaptic_row_t row,
    ...
    for (; fixed_synapse > 0; fixed_synapse--) {
        ...
        uint32_t combined_synapse_neuron_index =
            synapse_row_sparse_type_index(synaptic_word);

        log_debug("Neuron idx=%d receives payload = 0x%08x",
...
//!        based on (if the model was compiled with SYNAPSE_BENCHMARK parameter)
//!        or returns 0
