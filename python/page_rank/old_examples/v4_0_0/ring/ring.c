...
// Graph definition - 4 nodes ABCD

#define NODE_A      1
#define NODE_B      2
#define NODE_C      3
#define NODE_D      4
#define NODE_COUNT  NODE_D

#define MASK_NODE_A (1 << NODE_A)
#define MASK_NODE_B (1 << NODE_B)
#define MASK_NODE_C (1 << NODE_C)
#define MASK_NODE_D (1 << NODE_D)

#define OUT_EDGES_A (MASK_NODE_B | MASK_NODE_C)
#define OUT_EDGES_B MASK_NODE_D
#define OUT_EDGES_C (MASK_NODE_A | MASK_NODE_B | MASK_NODE_D)
#define OUT_EDGES_D MASK_NODE_C

uint out_edges[] = {OUT_EDGES_A, OUT_EDGES_B, OUT_EDGES_C, OUT_EDGES_D};
...
void c_main (void) {
  ...
  // Say hello...
  if (NODE_A <= core && core <= NODE_COUNT) {
      io_printf (IO_BUF, "# App \"%s\" on core %d (AppID %d)\n", app_name, core, app_id);
  } else {
      io_printf (IO_BUF, "# App \"%s\" does not need core %d - quit.\n", app_name, core);
      return;
  }

  if (core == 1) {
    sark_led_init();
    sark_led_set(LED_FLIP(1));
  }

  // Initialise User variables
  sark.vcpu->user0 = sark.vcpu->user1 = 0;

  // Set up a single MC router entry to send packets to the next core in the ring
//  uint node_out_edges = out_edges[core - 1];
//  while (node_out_edges > 0) {
//
//
//  }
  ...
  uint next = (core == NODE_COUNT) ? 1 : core + 1;
  uint next2 = (next == NODE_COUNT) ? 1 : next + 1;
  rtr_mc_set(e, core, 0xffffffff, MC_CORE_ROUTE(next));
  rtr_mc_set(e+1, core, 0xffffffff, MC_CORE_ROUTE(next2));
