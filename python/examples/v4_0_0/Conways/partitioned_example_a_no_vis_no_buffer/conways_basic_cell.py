...
class ConwayBasicCell(
    ...
    def generate_machine_data_specification(
        ...
        if not utility_calls.is_single(partitions):
            raise exceptions.ConfigurationException("Can only handle one type of partition. ")

        # check for duplicates
        edges = list(machine_graph.get_edges_ending_at_vertex(self))
        if len(set(edges)) != 8:
            output = " : ".join([e.pre_subvertex.label for e in edges])
        ...
        if len(edges) != 8:
            raise exceptions.ConfigurationException(
                ...
                "I have {} instead of 9".format(len(machine_graph.incoming_subedges_from_subvertex(self))))

        for edge in edges:
            if edge.pre_vertex == self:
                raise exceptions.ConfigurationException(
                    "I'm connected to myself, this is deemed an error please fix.")

        # write key needed to transmit with
        key = routing_info.get_first_key_from_pre_vertex(self, self.PARTITION_ID)
    ...
    def get_data(self, transceiver, placement, n_machine_time_steps):
        ...
        record_region_base_address = helpful_functions.locate_memory_region_for_placement(
            placement, self.DATA_REGIONS.RESULTS.value, transceiver)

        # find how many bytes are needed to be read
        number_of_bytes_to_read = str(transceiver.read_memory(placement.x, placement.y, record_region_base_address, 4))
        number_of_bytes_to_read = struct.unpack("<I", number_of_bytes_to_read)[0]

        # read the bytes
        if number_of_bytes_to_read != (n_machine_time_steps * 4):
            raise exceptions.ConfigurationException("number of bytes seems wrong")

        raw_data = str(
            transceiver.read_memory(placement.x, placement.y, record_region_base_address + 4, number_of_bytes_to_read)
        )

        # convert to ints
        elements = struct.unpack("<{}I".format(n_machine_time_steps), raw_data)
        data = [element != 0 for element in elements]
