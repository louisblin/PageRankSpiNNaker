...
class TemplateVertex(
    ...
    def __init__(self, label, constraints=None):
        ...
        if config.getboolean("Buffers", "enable_buffered_recording"):
            self._buffer_size_before_receive = config.getint("Buffers", "buffer_size_before_receive")
        self._time_between_requests = config.getint("Buffers", "time_between_requests")
        self._receive_buffer_host = config.get("Buffers", "receive_buffer_host")
        self._receive_buffer_port = helpful_functions.read_config_int(config, "Buffers", "receive_buffer_port")
    ...
    def resources_required(self):
        resources = ResourceContainer(
            ...
            sdram=SDRAMResource(constants.SYSTEM_BYTES_REQUIREMENT + self.TRANSMISSION_REGION_N_BYTES))
        resources.extend(recording_utilities.get_recording_resources(
            [self._recording_size], self._receive_buffer_host, self._receive_buffer_port))
        return resources
    ...
    def generate_machine_data_specification(self, spec, placement, machine_graph, routing_info, iptags,
    ...
    def read(self, placement, buffer_manager):
        ...
        """
        data_pointer, is_missing_data = buffer_manager.get_data_for_vertex(placement, 0)
        if is_missing_data:
