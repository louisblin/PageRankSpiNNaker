...
class PageRankVertexPartitioned(PartitionedVertex, AbstractPartitionedDataSpecableVertex, ReceiveBuffersToHostBasicImpl):
    """ A vertex partition for a page rank; represents a page rank element.
    """
    ...
    def __init__(self, label, machine_time_step, time_scale_factor, heat_temperature=0, constraints=None):

        # resources used by a page rank element vertex
        sdram = SDRAMResource(23 + config.getint("Buffers", "minimum_buffer_sdram"))
        resources = ResourceContainer(cpu=CPUCyclesPerTickResource(45), dtcm=DTCMResource(34), sdram=sdram)

        ReceiveBuffersToHostBasicImpl.__init__(self)
        PartitionedVertex.__init__(self, label=label, resources_required=resources, constraints=constraints)
        AbstractPartitionedDataSpecableVertex.__init__(self, machine_time_step, time_scale_factor)
    ...
    def generate_data_spec(self, placement, sub_graph, routing_info, hostname, report_folder, ip_tags, reverse_ip_tags,
        ...
        :param reverse_ip_tags: the collection of reverse iptags generated by the tag allocator
        :param report_folder: the folder to write reports to
        :param write_text_specs: bool which says if test specs should be written
        :param application_run_time_folder: the folder where application files are written
        """
        data_writer, report_writer = \
            self.get_data_spec_file_writers(placement.x, placement.y, placement.p, hostname, report_folder,
                write_text_specs, application_run_time_folder)
    ...
    def _reserve_memory_regions(self, spec, system_size):
        spec.reserve_memory_region(
            region=self.DATA_REGIONS.SYSTEM.value, size=system_size, label='systemInfo')
        spec.reserve_memory_region(
            region=self.DATA_REGIONS.TRANSMISSIONS.value, size=self.TRANSMISSION_DATA_SIZE, label="inputs")
        spec.reserve_memory_region(
            region=self.DATA_REGIONS.NEIGHBOUR_KEYS.value, size=self.NEIGHBOUR_DATA_SIZE, label="inputs")
        spec.reserve_memory_region(
            region=self.DATA_REGIONS.COMMAND_KEYS.value, size=self.COMMAND_KEYS_SIZE, label="commands")
        spec.reserve_memory_region(
            region=self.DATA_REGIONS.TEMP_VALUE.value, size=self.TEMP_VALUE_SIZE, label="temp")
    ...
    def _write_key_data(self, spec, routing_info, sub_graph):
        ...
        spec.comment("\n the keys for the neighbours in DIRECTED. order:\n\n")
        direction_edges = list()
        fake_temp_edges = list()
        command_edge = None
        output_edge = None
        for incoming_edge in incoming_edges:
            if isinstance(incoming_edge, PageRankEdge) and \
               isinstance(incoming_edge.pre_subvertex, ReverseIpTagMultiCastSource):
                fake_temp_edges.append(incoming_edge)

            elif isinstance(incoming_edge, PageRankEdge) and \
                 isinstance(incoming_edge.pre_subvertex, PageRankVertexPartitioned):
        ...
        direction_edges = sorted(
            direction_edges, key=lambda subedge: subedge.direction.value, reverse=False)

        # write each key that this module should expect packets from in order of DIRECTED
        loaded_keys = 0
        for current_direction in range(1):
            written = False
        ...
        if loaded_keys == 0:
            raise exceptions.ConfigurationException(
                "This page rank element {} does not receive any data from other elements. Please fix and try again.  "
                "It currently has incoming edges of {} directional edges of {} and fake edges of {} and command edge "
                "of {} and output edge of {}".format(self.label, incoming_edges, direction_edges, fake_temp_edges,
                                                     command_edge, output_edge))

        # write each key that this model should expect packets from in order of DIRECTED for injected temperatures
